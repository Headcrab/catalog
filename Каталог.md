...
if (catalogPages.length === 0) {
    ...
    function parseItems(content, filePath) {
        ...
        content.split('\n').forEach(line => {
            ...
            if (line.startsWith('- Товар')) {
                ...
                currentItem[key.replace('- ', '')] = value.split(',').map(v => v.trim());
            }
        });
    ...
    // Добавьте эту ф��нкцию после объявления переменной allItems
    ...
    if (allItems.length === 0) {
        ...
        const filterContainer = mainContainer.createEl("div", {attr: {style: "margin-bottom: 20px; display: flex; gap: 10px; align-items: center;"}});
        ...
        async function createLinkOrText(container, linkPath, displayText) {

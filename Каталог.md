...
if (catalogPages.length === 0) {
    ...
    if (allItems.length === 0) {
        ...
        async function createLinkOrText(container, value, displayText) {
            ...
            const previewSize = currentFileProperties.preview_size || 100; // Получаем размер превью из свойств файла
            ...
            if (isMedia && file) {
                ...
                const imgElement = container.createEl("img", {
                    attr: {
                        ...
                        alt: linkText,
                        style: `max-width: ${previewSize}px; max-height: ${previewSize}px; width: auto; height: auto; border-radius: 4px; object-fit: contain;`
                    }
                });

                // Добавляем обработчик для открытия изображения в полном размере при клике

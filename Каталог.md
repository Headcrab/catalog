...
if (catalogPages.length === 0) {
    ...
    if (allItems.length === 0) {
            ...
            items.forEach((item, index) => {
                ...
                ["Название", "Цвет", "Вес", "Класс"].forEach(key => {
                    ...
                    if (typeof value === 'string' && value.startsWith("[[") && value.endsWith("]]")) {
                        const linkText = value.slice(2, -2); // Убираем [[ и ]]
                        createLinkOrText(cell, linkText);
                    } else {
        ...
        function createLinkOrText(container, text) {
            const file = app.vault.getAbstractFileByPath(text + '.md');
            
            const linkElement = container.createEl("a", {
                text: text, // Отображаем только текст без [[ и ]]
                attr: {
                    href: "#",
        ...
        async function createNewFile(fileName) {
            ...
            try {
                ...
                new Notice(`Файл "${fileName}" успешно создан`);
